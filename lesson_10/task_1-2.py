...
def task_4(matrice):
    return [item for row in matrice for item in row]
# CODUL TĂU VINE MAI SUS:
...
# CODUL TĂU VINE MAI JOS:
def task_6(n: int) -> dict:
    return {key: value for key, value in zip([i for i in range(1, n + 1)], [i ** 3 for i in range(1, n + 1)])}
# CODUL TĂU VINE MAI SUS:
...
def task_8(numbers: list[int]) -> float:
...
def task_14(varsta):
    return "minor" if varsta < 18 else "adult" if varsta <= 65 else "senior"
# CODUL TĂU VINE MAI SUS:
...
# CODUL TĂU VINE MAI JOS:
def task_20(numar):
    return numar == sum([i for i in range(1, numar) if numar % i == 0])
# CODUL TĂU VINE MAI SUS:
...
def task_21(numar):
    numbers = list(map(int, str(numar)))
    rezult = sum([i * i * i for i in numbers])
    return numar == rezult
# CODUL TĂU VINE MAI SUS:
