...
class Utilizator:
    def __init__(self, nume, nivel_acces = "Default") -> None:
        self.nume = nume
...
class UtilizatorManager(Utilizator):
    def __init__(self, nume, nivel_acces="Manager") -> None:
...
class UtilizatorAdmin(Utilizator):
    def __init__(self, nume, nivel_acces="Admin") -> None:
        super().__init__(nume, nivel_acces)
...
class user:
    ...
    def set_nivel_access(self, nivel_acces):
        self.__nivel_acces = nivel_acces
...
class Sistem:
    ...
    def adauga_utilizator(self, utilizator: Utilizator):
        id = len(self.__utilizatori) + 1
    ...
    def verifica_nivel_acces(self, nume):
        for utilizator in self.__utilizatori.values():
            if utilizator.get_nume() == nume:
                return utilizator.get_nivel_acces()
        return "Utilizatorul nu exista"

    def modifica_name_user(self, id, nume):
        self.__utilizatori[id].set_nume(nume)

    def sterge_utilizator(self, id):
        self.__utilizatori.pop(id)

    def modifica_nivel_acces(self, id, nivel_acces):
        self.__utilizatori[id].set_nivel_access(nivel_acces)
...
class TechSolutionsApp:
    ...
    def __init__(self, versiune_aplicatie = "1.0") -> None:
